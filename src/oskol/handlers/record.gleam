...
fn stored_json(
  ...
  // A newer completed-game marker means the rows are missing a result.
  // Ordinary moves in an unfinished game do not invalidate finished games.
  use _ <- result.try(case !setup.records_stale {
    True -> Ok(Nil)
    False -> Error(Nil)
  })
