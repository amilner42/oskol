...
pub fn new(
  ...
  // On a tie the first picker is chosen at random
  let #(#(first_player, second_player), rng) = case round_was_tie {
    True -> {
      let #(flip, rng) = rng.chance(rng, 1, 2)
      case flip {
        True -> #(#(winner_id, loser_id), rng)
        False -> #(#(loser_id, winner_id), rng)
      }
    }
    False -> #(#(winner_id, loser_id), rng)
  }
...
pub fn confirm_deck_builder_pick(
  ...
  rng: Rng,
) -> Result(#(ShopState, Rng), ShopError) {
          ...
          case selected_card.kind {
            shop_card.Logistics(_) -> {
              ...
              case mark_card_picked(shop_state, card_id) {
                Error(e) -> Error(e)
                Ok(updated_state) -> {
                  // Sample 8 cards from the player's whole deck for selection
                  let #(available_cards, rng) = rng.sample(rng, player_hand, 8)
          ...
          case selected_card.kind {
            shop_card.Sabotage(shop_card.PlusBomb(_)) -> {
              ...
              case mark_card_picked(shop_state, card_id) {
                Error(e) -> Error(e)
                Ok(updated_state) -> {
                  // Sample 8 cards from the opponent's whole deck for selection
                  let #(available_cards, rng) = rng.sample(rng, player_hand, 8)
