/**
 * End-to-end test for rank-increasing action cards
 *
 * Flow:
 * 1. Two players join and start game
 * 2. Play through one complete round (4 hands)
 * 3. Shop: Primary attempts rank-increasing card, others pick level ups
 * 4. Both players ready up
 * 5. View Cards modal to verify rank increases
 */
...
async function main() {
  ...
  try {
    ...
    await sleep(2000);
    await primary.screenshot({ path: 'playwright/screenshots/test-rank-increase-action/02-player1-joined.png', fullPage: true });
    log('✓ Primary joined');
    ...
    await primary.screenshot({ path: 'playwright/screenshots/test-rank-increase-action/03-both-joined-p1.png', fullPage: true });
    await opponent.screenshot({ path: 'playwright/screenshots/test-rank-increase-action/04-both-joined-p2.png', fullPage: true });
    ...
    await primary.screenshot({ path: 'playwright/screenshots/test-rank-increase-action/05-game-started-p1.png', fullPage: true });
    await opponent.screenshot({ path: 'playwright/screenshots/test-rank-increase-action/06-game-started-p2.png', fullPage: true });
    log('✓ Game started');
    ...
    await primary.screenshot({ path: 'playwright/screenshots/test-rank-increase-action/07-round-complete-p1.png', fullPage: true });
    await opponent.screenshot({ path: 'playwright/screenshots/test-rank-increase-action/08-round-complete-p2.png', fullPage: true });
    ...
    await primary.screenshot({ path: 'playwright/screenshots/test-rank-increase-action/09-shop-p1.png', fullPage: true });
    await opponent.screenshot({ path: 'playwright/screenshots/test-rank-increase-action/10-shop-p2.png', fullPage: true });
    ...
    if (firstPickerIsPrimary1) {
      // Primary tries to find rank increase card
      const deckBuilderCards = await firstPicker1.$$('[phx-click="preview_deck_builder"]');
      let foundRankCard = false;

      for (let i = 0; i < deckBuilderCards.length; i++) {
        const cardText = await deckBuilderCards[i].textContent();
        if (cardText.includes('Increase Rank')) {
          log(`  ✓ Found rank increase card!`);
          foundRankCard = true;

          await deckBuilderCards[i].click();
          await sleep(2000);
          await firstPicker1.screenshot({ path: 'playwright/screenshots/test-rank-increase-action/11-rank-card-preview.png', fullPage: true });

          const confirmBtn = await firstPicker1.$('button:has-text("Confirm Pick")');
          if (confirmBtn) {
            await confirmBtn.click();
            await sleep(2500);
            await firstPicker1.screenshot({ path: 'playwright/screenshots/test-rank-increase-action/12-rank-card-selection.png', fullPage: true });
            ...
            await firstPicker1.screenshot({ path: 'playwright/screenshots/test-rank-increase-action/13-cards-selected.png', fullPage: true });

            const confirmSelection = await firstPicker1.$('button:has-text("Confirm Selection")');
            if (confirmSelection) {
              await confirmSelection.click();
              await sleep(2500);
              await firstPicker1.screenshot({ path: 'playwright/screenshots/test-rank-increase-action/14-after-rank-increase.png', fullPage: true });
              log(`  ✓ Primary picked rank increase card`);
            }
          }
          break;
        }
      ...
      if (!foundRankCard) {
        log(`  ⚠ No rank increase card available, picking another shop card`);
    ...
    if (!firstPickerIsPrimary1) {
      ...
      for (let i = 0; i < deckBuilderCards.length; i++) {
        ...
        if (cardText.includes('Increase Rank')) {
          ...
          await deckBuilderCards[i].click();
          await sleep(2000);
          await secondPicker1.screenshot({ path: 'playwright/screenshots/test-rank-increase-action/11-rank-card-preview.png', fullPage: true });

          const confirmBtn = await secondPicker1.$('button:has-text("Confirm Pick")');
          if (confirmBtn) {
            await confirmBtn.click();
            await sleep(2500);
            await secondPicker1.screenshot({ path: 'playwright/screenshots/test-rank-increase-action/12-rank-card-selection.png', fullPage: true });
            ...
            await secondPicker1.screenshot({ path: 'playwright/screenshots/test-rank-increase-action/13-cards-selected.png', fullPage: true });

            const confirmSelection = await secondPicker1.$('button:has-text("Confirm Selection")');
            if (confirmSelection) {
              await confirmSelection.click();
              await sleep(2500);
              await secondPicker1.screenshot({ path: 'playwright/screenshots/test-rank-increase-action/14-after-rank-increase.png', fullPage: true });
              log(`  ✓ Primary picked rank increase card`);
            }
          }
          break;
    ...
    await primary.screenshot({ path: 'playwright/screenshots/test-rank-increase-action/15-next-round-started.png', fullPage: true });
    await opponent.screenshot({ path: 'playwright/screenshots/test-rank-increase-action/16-next-round-started-p2.png', fullPage: true });
    ...
    if (viewCardsBtn) {
      ...
      await sleep(1500);
      await primary.screenshot({ path: 'playwright/screenshots/test-rank-increase-action/17-view-cards-modal.png', fullPage: true });
      log('✓ Screenshot: 17-view-cards-modal.png');
      log('✓ Check this screenshot to verify rank increases on selected cards');
    } else {
      log('⚠ View Cards button not found');
      await primary.screenshot({ path: 'playwright/screenshots/test-rank-increase-action/ERROR-no-view-cards.png', fullPage: true });
    }
    ...
    try {
      ...
      if (pages.length > 0) {
        await pages[0].screenshot({ path: 'playwright/screenshots/test-rank-increase-action/ERROR.png', fullPage: true });
