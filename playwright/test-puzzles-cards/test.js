...
async function run(browser, setup, errors) {
  ...
  try {
    ...
    // The end is the summary and the way back: nothing is fetched, and
    // nothing on it starts more.
    await a.waitForSelector('#pz-home', { timeout: 15000 });
    must(!(await a.locator('#pz-signin-ask').count()), 'and ends on the summary');
