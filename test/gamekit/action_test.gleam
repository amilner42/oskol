...
pub fn params_are_read_with_their_types_test() {
  ...
  assert action.optional_string_param(p, "from") == Ok(Some("13"))
  assert action.optional_string_param(p, "missing") == Ok(None)
  assert action.optional_string_param(p, "n")
    == Error("Missing or invalid param: n")
