defmodule OskolWeb.GameChannelTest do
  ...
  # The guest id is the credential, and the real socket reads it off the
  # session the websocket's upgrade request carried; here it is assigned
  # straight onto the socket, which is the same thing one step earlier.
  ...
  defp try_join(game_id, assigns) do
    OskolWeb.UserSocket
    |> socket("user", assigns)
    |> subscribe_and_join(OskolWeb.GameChannel, "game:#{game_id}", %{})
  end
  ...
  # see.
  defp join_as(game_id, guest_id, client, opts \\ []) do
    socket = socket(OskolWeb.UserSocket, "user", %{guest_id: guest_id, client: client})
    ...
    {:ok, reply, joined} =
      subscribe_and_join(socket, OskolWeb.GameChannel, "game:#{game_id}", %{})

    {reply, joined}
  end

  # A join from another browser holding the same seat -- the same guest, a
  # different tab. Every socket in a test shares the test process as its
  # transport, which is what the room falls back to reading as "the same
  # client", so a second tab has to be spelled out.
  defp join_as_other_client(game_id, guest_id) do
    other_browser = spawn(fn -> Process.sleep(:infinity) end)

    {:ok, reply, socket} =
      %{socket(OskolWeb.UserSocket, "user", %{guest_id: guest_id}) | transport_pid: other_browser}
      |> subscribe_and_join(OskolWeb.GameChannel, "game:#{game_id}", %{})
  ...
  test "joining a running game returns this player's scene and legal actions" do
    %{game_id: game_id, mover: mover, mover_guest: mt, waiting_guest: wt} = started()
  ...
  test "an action pushes an update with events to everyone and errors only to the actor" do
    %{game_id: game_id, state: state, mover: mover, mover_guest: mt, waiting_guest: wt} =
      started()
  ...
  test "a join from a browser with no guest cookie is refused" do
    ...
    assert {:error, %{reason: "unauthorized"}} = try_join(game_id, %{guest_id: nil})
    assert {:error, %{reason: "unauthorized"}} = try_join(game_id, %{guest_id: ""})
  end

  test "a join from a guest who holds no seat here is refused" do
    %{game_id: game_id, p1: p1} = started()
    assert {:error, %{reason: "unauthorized"}} = try_join(game_id, %{guest_id: "a-stranger"})
    # A player id is public; it is not a credential, and neither is a name.
    assert {:error, %{reason: "unauthorized"}} = try_join(game_id, %{guest_id: p1})
  ...
  test "a guest who held a seat stops getting in once it is claimed away" do
    ...
    %{game_id: game_id, p1: p1, g1: was_alice} = started()
    {:ok, ^p1, _} = Game.claim_seat(game_id, p1, self(), "another-browser")
    assert {:error, %{reason: "unauthorized"}} = try_join(game_id, %{guest_id: was_alice})
    {reply, _socket} = join_room(game_id, "another-browser")
    assert reply.payload.player_id == p1
  end

  test "a room in its lobby is joined by the guest who took the seat" do
    # A seat taken with no guest at all (tooling, a seeded room) is held by
    # nobody, and no browser walks into it: it is claimed from the invite.
    %{game_id: game_id} = lobby()
    {:ok, _, _} = Game.join_game(game_id, "Nobody", nil, nil)
    assert {:error, %{reason: "unauthorized"}} = try_join(game_id, %{guest_id: nil})
  end
  ...
  test "the same client rejoining its seat resumes quietly" do
    ...
    {_, first} = join_room(game_id, g1)
    Process.unlink(first.channel_pid)
    flush_updates()

    {reply, second} = join_room(game_id, g1)
    assert reply.payload.player_id == p1
  ...
  test "another tab of the same guest takes the seat over and the old one is told" do
    ...
    {reply, _second} = join_as_other_client(game_id, g1)
    assert reply.payload.player_id == p1
    assert_push "error", %{message: "This seat was opened somewhere else"}
    assert_receive {:DOWN, ^ref, :process, _, _}, 1000
