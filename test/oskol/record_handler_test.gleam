...
pub fn a_token_that_opens_no_seat_reads_nothing_test() {
  ...
  assert record.record_json(ctx, "backgammon", "000007", "stolen")
    == Error(error.NotFound(record.not_found_message))
}

pub fn no_token_never_reaches_the_room_test() {
  // The seat cap panics if asked: an empty token is refused before it.
  let ctx =
    fakes.ctx()
    |> fakes.with_room(Some(fakes.room()), None)
    |> fakes.with_slug(Some("backgammon"))
  assert record.record_json(ctx, "backgammon", "000007", "")
    == Error(error.NotFound(record.not_found_message))
}
