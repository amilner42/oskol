defmodule Oskol.Game.ShopCard do
  ...
  @doc """
  Generates random level up cards with weighted frequencies.
  """
  @spec generate_random_level_ups(pos_integer()) :: [t()]
  def generate_random_level_ups(count) do
    pool =
      [
    ...
    |> Enum.shuffle()
    |> Enum.take(count)
    |> Enum.map(fn hand_type -> %__MODULE__{type: :level_up, subtype: hand_type} end)
  end

  @doc """
  Generates random deck builder cards.
  """
  @spec generate_random_deck_builders(pos_integer()) :: [t()]
  def generate_random_deck_builders(count) do
  ...
  def generate_random_sabotage_cards(count, dev_codes \\ []) do
    ...
    forced_cards =
      []
      |> then(fn cards ->
        if "SHOP_FORCE_SCRAMBLER" in dev_codes, do: [scrambler_card() | cards], else: cards
      end)
      |> then(fn cards ->
        if "SHOP_FORCE_PLUS_BOMB" in dev_codes, do: [plus_bomb_card() | cards], else: cards
      end)
      |> then(fn cards ->
